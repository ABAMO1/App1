...
def show_analysis_results(data, result, bmi, bmi_category):
    ...
    try:
        ...
        st.markdown("<div class='section-header'>📋 التوصيات العامة</div>", unsafe_allowhtml=True)
...
# في الجزء الخاص بإرسال البيانات، نقوم بتحديث عنوان API
        ...
        if not is_valid:
            ...
            with st.spinner('جاري تحليل البيانات... ⏳'):
                response = requests.post(
                    "https://myapp-3-wg9g.onrender.com/submit-symptoms/",
                    json=data,
                    timeout=60
                )
